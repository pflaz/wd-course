...
export function thumbUpPost(cuid, votes) {
  return {
    type: THUMB_UP_POST,
    cuid,
    votes
...
export function thumbUpRequest(cuid) {
  return(dispatch) => {
    return callApi(`posts/thumbUp/${cuid}`, 'put').then(votes => dispatch(thumbUpPost(cuid, votes)));
  };
}

export function thumbDownPost(cuid, votes) {
  return {
...
export function thumbDownRequest(cuid) {
  return(dispatch) => {
    return callApi(`posts/thumbDown/${cuid}`, 'put').then(votes => dispatch(thumbDownPost(cuid, votes)));
  };
}